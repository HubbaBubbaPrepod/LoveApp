...
const ENTITY_MAP = {
  ...
  event:         { table: 'calendar_events',  ownerCol: 'user_id', partnerRead: true  },
...
async function upsertEntity(userId, entityType, meta, data) {
  // Build a generic upsert from data columns, excluding protected fields
  const EXCLUDED = new Set(['id', 'user_id', 'server_updated_at', 'created_at', 'deleted_at']);

  const columns  = Object.keys(data).filter((k) => !EXCLUDED.has(k));
  ...
  if (data.id) {
    ...
    const res = await pool.query(
      `UPDATE ${meta.table}
       SET ${setClause}, server_updated_at = NOW(), updated_at = NOW()
       WHERE id = $1 AND ${meta.ownerCol} = $2
       RETURNING *`,
    ...
    // INSERT
    const allColumns  = [`${meta.ownerCol}`, ...columns];
